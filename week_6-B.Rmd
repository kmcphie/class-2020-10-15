...
```{r}
x <- tibble(urn_red = 0:1000) %>% 
  mutate(paddle_red = map(urn_red, ~ rbinom(n = 10000, size = 25, prob = ./1000))) %>% 
  unnest(paddle_red)
...
post %>%
  mutate(new_reds = map(urn_red, ~ rbinom(n = 10000, size = 20, prob = ./1000))) %>%
  unnest(new_reds) %>%
  mutate(gt_3 = new_reds > 3) %>%
  mutate(gt3_weighted = gt_3 * prob) %>%
  summarize(final = sum(gt3_weighted) / sum(prob))
